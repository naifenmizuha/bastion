...
func TestParsePositions(t *testing.T) {
	got, err := ParsePositions(" pitcher,first_base, shortstop,pitcher ")
	if err != nil {
		t.Fatalf("ParsePositions failed: %v", err)
	}
	if got != PositionPitcher|PositionFirstBase|PositionShortstop {
		t.Fatalf("unexpected positions: %q", got)
	}
	if FormatPositions(got) != "pitcher,first_base,shortstop" {
		t.Fatalf("unexpected formatted positions: %q", FormatPositions(got))
	}
}

func TestParsePositionsRejectsInvalidValue(t *testing.T) {
	_, err := ParsePositions("infield")
	if err == nil {
