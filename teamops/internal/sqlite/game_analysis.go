...
func (s *Store) getAnalysisSummaries(gameID int64, player string) ([]game.PlayerPerformanceSummary, error) {
	query := `
SELECT id, game_id, team_id, player, batting_order, positions, batting_available, baserunning_available,
...
func (s *Store) getBattingStats(gameID int64, player string) ([]game.PlayerBattingStats, error) {
	query := `
SELECT id, game_id, team_id, player, pa, at_bats, hits, singles, doubles, triples, homeruns, walks,
...
func (s *Store) getBaserunningStats(gameID int64, player string) ([]game.PlayerBaserunningStats, error) {
	query := `
SELECT id, game_id, team_id, player, runs, stolen_bases, caught_stealing, stolen_base_attempts,
...
func (s *Store) getPitchingStats(gameID int64, player string) ([]game.PlayerPitchingStats, error) {
	query := `
SELECT id, game_id, team_id, player, outs_recorded, innings_pitched, batters_faced, hits_allowed,
...
func (s *Store) getFieldingStats(gameID int64, player string) ([]game.PlayerFieldingStats, error) {
	query := `
SELECT id, game_id, team_id, player, positions, putouts, assists, errors, total_chances,
