...
import { TeamOpsService, type TeamOpsRunner } from "./service.ts";
...
describe("TeamOpsService", () => {
  it("returns the matching command contract for invalid structured input without execution or approval", async () => {
    ...
    await assert.rejects(
      new TeamOpsService(runner).execute(
        { args: ["player", "add"] },
        {
          confirmWrite: async () => {
            confirmations += 1;
      ...
      (error: unknown) => {
        assert.ok(error instanceof TeamOpsError);
  ...
  it("requires and respects write approval before execution", async () => {
    ...
    const service = new TeamOpsService(runner);
  ...
  it("preflights game events before approval and returns compact issues", async () => {
    ...
    const details = await new TeamOpsService(runner).execute(
      {
