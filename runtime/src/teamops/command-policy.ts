...
export function parseCommand(params: TeamOpsParams): ParsedCommand {
  ...
  for (let index = 0; index < flagTokens.length; index += 2) {
    ...
    if (flags.has(name)) {
      throw new TeamOpsError("INVALID_FLAGS", `duplicate flag: ${name}`);
    }
    if (value.length === 0 || value.startsWith("--")) {
      throw new TeamOpsError("INVALID_FLAGS", `flag ${name} requires a value`);
    }
