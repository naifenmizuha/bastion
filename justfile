...
# Run the fixed four-turn runtime scenario against an isolated temporary DB.
rt-scenario-test: build
    cd {{runtime_dir}} && pnpm scenario-test

# Run the data-driven real-provider evaluation suite.
rt-eval *args: build
    cd {{runtime_dir}} && node --import tsx src/eval/main.ts {{args}}
