...
reset-athletics-2025: go-build
    ...
    ./{{bin}} --help | grep -q '^  team list' || { echo 'athletics-2025.sql requires the multi-user TeamOps schema; switch to codex/multi-user-derived-memory first.' >&2; exit 1; }
    rm -f bastion.db
    ./{{bin}} --db bastion.db team list >/dev/null
