...
每条依赖保存权威实体的 `updated_at` 快照。Runtime 重启后在检索或读取时重新计算
快照；来源未变化的记忆继续保持 fresh，来源变化的记忆转为 stale。
...
Runtime 必须从当前会话的已验证读取账本（Verified Read Ledger）验证每条依赖：
...
`search` 支持按 kind、subject、topic、文本和 scope 筛选，默认仅返回 fresh 记忆和
有界摘要。Store 分别查询 private、staff 和 team，工具层完成 freshness 校验、稳定
排序、ID 去重和一次全局 limit，不使用 SQL `UNION`。调用者必须显式设置
`includeStale` 才能搜索 stale 记录。

`read` 返回完整结论、限制、状态和依赖。stale 记录必须附带“重新执行全部依赖后再
形成替代结论”的警告。

fresh 记录在返回前必须完成来源快照校验。校验不可用时本次状态为 unknown，不得作为
fresh 使用，也不得永久把记录标记为 stale。

第一版不做 embedding 或自动重算。每个非命令用户轮次可以自动注入最多 3 条 fresh
候选的有界目录；候选按当前 prompt 的确定性文本相关度和更新时间排序，只包含 ID、
类型、标签、最多 400 字的摘要、限制和状态。完整结论仍必须通过 `read` 按需进入上下文。
stale 或 freshness unknown 的记录不得作为候选注入。
...
每个读命令由声明式注册表映射到权威实体及其 `updated_at`。每个成功的 write 或
compute-write 触发即时快照复检；检索和读取也必须复检。实体集合或时间戳变化时，
fresh 记忆转换为 stale。
...
派生记忆完整正文只通过工具按需进入上下文；Runtime 可以在每轮开始前注入有界候选
目录。候选直接覆盖只读分析问题时，Agent 应读取该记忆并复用，不得仅为复核依赖而
重新查询 CLI。没有相关 fresh 候选、用户明确要求重新核查、问题超出结论或限制，或
需要领域写入/current-state precondition 时，才回退权威 CLI。已完成轮次中的 memory
tool trace 按普通工具轨迹由 Context Projection 移除；持久化记录不受影响。
...
2. fresh 结论可跨会话检索；每轮最多自动注入 3 条 400 字摘要候选，完整正文不会自动
   污染 Provider 上下文。
...
8. 进程重启后来源未变化的 fresh 记录仍可复用；旧 schema 明确拒绝启动。
9. Session、CLI、compaction 与 projection 原有测试无回归。
